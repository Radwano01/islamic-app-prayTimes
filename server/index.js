...
app.get("/api/:country/:city/:month/:day/:weekday", async (req, res) => {
  ...
  try {
    ...
    if (!response.data) {
      res.status(404).json("No data available from the external API.");
      return;
    }
    ...
    if (Data[country] && Data[country][city]) {
        ...
        if (cityTimes) {
          if (
            ...
            res
              .status(404)
              .json(
                "No prayer times available for the specified date and weekday."
              );
          }
        } else {
      ...
      res
        .status(404)
        .json("No data available for the specified country and city.");
    }
  } catch (error) {
